...
namespace Aurigin
{
	...
	public class UserSyringeControl
	{
		...
		private int InitializePlungerAndValve(byte pumpAddress)
		{
			...
			{
				command = "Y" + pumpConfiguration.SyringeParams[index].InitializationForceCode_0to3 + "R";
			}
			else
			{
				command = "Z" + pumpConfiguration.SyringeParams[index].InitializationForceCode_0to3 + "R";
			}
...
//				profile = new Aurigin.Xml(@"..\data\commPort.xml");
				...
				{
					for (int i = 1; i <= pumpConfiguration.TotalSyringePump; i++)
					{
...
//					Comm.DTR(port[0], true);
		...
		private int ClearResponseBuffer(string pumpAddress)
		{
			try
			{
				...
				if (!pumpConfiguration.MultiplexPump)
				{
					Comm.ClearReadBuffer(pumpConfiguration.SyringeParams[Convert.ToInt16(pumpAddress)-1].PumpCommPort);
				}
				else
				{
					Comm.ClearReadBuffer(port[0]);
				}
			}
			catch
			{
...
//			byte temp = 0x30;
		...
		{		
			...
			try
			{
				...
				if (!pumpConfiguration.MultiplexPump)
				{
//					Comm.Write(port[Convert.ToInt16(pumpAddress)-1], cmd);
		...
		// Syringe-TODO - This is very odd that it waits for a response from any pump.
