...
namespace EA.PixyControl
{
	...
	public class ServoControl
	{
        ...
        // PKv5.5.5.2  Lets try to make this quite a bit shorter..   Query commands should get right back to us,  5000 msec was eternity.
        // private const int COMMAND_TIMEOUT = 5000;  // Up to how long to wait when a command is supposed to respond with data msec
        private const int COMMAND_TIMEOUT = 500;  // Up to how long to wait when a command is supposed to respond with data msec
...
//		private bool				initialized = false;
        ...
        // PKv5.5.5.2,    
        //   Improved version of the SendCommand()
        //    incorporates an option to GetResponse()
        //   

        // In the previous GetResponse()  supressError would supress two types of errors:
        //  1 - A CR was received was received but that's all.
        //  2 - Timeout, did not even receive a CR.  This is the a serious error and should not be suppressed.
        //   
        //  This routine will be much smarter if it gets either type of error it will automatically do 1 retry sending the command before alerting the user
        //  it also stays infinetly in the loop trying to communicate with the motor... because after all if the motor is not getting the command ..proceeding 
        //  could cause serious tip damage.
        //
        //  Errors are logged to  @"..\LogFiles\CommErrorLog.txt",  even the ones that are automatically retried.

        //  Assumes ,   All commands will look for an initial CR in response to the command.
        //  If it is a query command such as "PR EF" then the getResponse should be set to true.  
        //     The method will look for a response followed by a CR.   If it times out... then the method will retry sending the command and looking for the response

        public static string SendCommand(string address, string command,bool getResponse,int timeout_ms)
        {
            string retString = "";
            ...
            while (!allGood)
            {
                ...
                if (ClearResponseBuffer(portIndex) != 0)
                ...
                // Lets keep track of the last command.
                lastCommand = string.Format("Command = {0}, Port (0=Tip,1=Grid) = {1}, Axis Address (1=X to 3=Z)= {2}", command, portIndex, address);

                // Read back the CR in response to the command,  set receivedCR to true if received, else fill up errorMessage1  and  errorMessage2 for logging.
                #region
                try
                {
                    // Assuming the last character is always a LF.
                    ports[portIndex].NewLine = "\n";                        // used to be "\r"
                    ports[portIndex].ReadTimeout = COMMAND_WAIT_FOR_CR;
                    ...
                    {
                        errorMessage1 = string.Format("Looking for CR to acknowledge command but reply length ={0}, Reply = {1}", retString.Length, retString); 
                    }
                }

                catch (Exception ex)       // Likely Timeout
                {
                    errorMessage1 = string.Format("Looking for CR to acknowledge command but instead got timeout exception = \n{0}",ex.ToString());
                ...
                if (receivedCR)
                {
                    ...
                    {
                        // Read back response which is terminated by CR, if we do not receive at lease one char with  fill up errorMessage1  and  errorMessage2 for logging.
                        #region
                        try
                        {
                            // Assuming the last character is always a LF.
                            ports[portIndex].NewLine = "\n";                        // used to be "\r"
                            ports[portIndex].ReadTimeout = timeout_ms;
                            retString = ports[portIndex].ReadLine();
                            ...
                            {
                                errorMessage1 = string.Format("Looking for (Response)CR but got null string", retString.Length, retString);
                            }
                        }

                        catch (Exception ex)       // Likely Timeout
                        {
                            errorMessage1 = string.Format("Looking for (Response)CR command but instead got timeout exception = \n{0}", ex.ToString());
                            retString = ports[portIndex].ReadExisting();  // Might help the recovery by clearing it out
                        }
                        #endregion
                    }
                ...
                if (allGood) return retString;          // Can only return from here.

                // Try one outomatic recovery,  Alert user if that fails too.
                tries = tries + 1;
                errorMessage2 = lastCommand;

                if (tries > 2)
                {
                    string errorHeader = string.Format("Communcating with motor failed: try = {0},  OK to retry", tries);
                    DialogResult status = MessageBox.Show(errorMessage1 + errorMessage2, "errorHeader", MessageBoxButtons.OK);
                    // Will infinetely try and keep alermting the user.
                }

                LoggingUtility(errorMessage1);
                LoggingUtility(errorMessage2);

                errorMessage1 = "";
                errorMessage2 = "";

            }
            // Will never reach here....  but the compiler complained.
            return retString;
        }

        // PKv5.5.5.2,  Added the LoggingUtility method.

        private static void LoggingUtility(string logMessage)  // Timestamping to disk method
        {
            Datalog log = new Datalog(@"..\LogFiles\CommErrorLog.txt");
            log.WriteLine(Datalog.Timestamp());
            log.WriteLine(logMessage);
            log.Close();
            Console.WriteLine("Comm Error LOG"+Datalog.Timestamp());
        ...
        // 1 based axis.  Axis 4 thru 6 are the grid robot on the 2nd com port
 ...
 //           Console.WriteLine("To Motor =>[{0}]{1}", address, command);
        ...
        public static string GetResponse(int portIndex, int timeout, bool supressError)
        {
            ...
            catch (Exception ex)                // This is the timeout !!
            {
                if (!supressError)
                {
        ...
        // PKv5.5.5.2
        /* NOT USED ANYMORE (I hope !!!) --  These were 3 axis machine versions.
 ...
 //             Console.WriteLine("From Motor <={0}", retString);
        ...
        End of stuff that i don't think is used anymore */
        ...
        // PKv5.5.5.2  TODO - Have not updated this routine yet to use the improved SendCommand() need to study a bit more
        //  only used at startup so not a priority.
 ...
 //              if (SendCommand(AxisAddress.ToString(), "VM 1000") != 0) return -1;      //4000 to 1000
        ...
        public int ArmTriggerOut(int Axis, double relativePosition_mm, bool useGridRobot, int pulseWidth)
        {
            ...
            int relativePosition_encCounts = Convert.ToInt32(relativePosition_mm / AxisParams[arrayIndex].DistResolution_mmPerEncCount);
            string command = string.Format("CW={0}", pulseWidth);           //  Use the passed in pulse width.
            SendCommand(AxisAddress.ToString(), command, false, 0);        //   PKv5.3.3.1,   Bug fix for version PKv5.3.3

            //          string command = string.Format("TR={0},0,1", relativePosition_encCounts); // Only one trigger output

            //  2019-01-24  Seems like it is all set for sending out many triggers in a row this is now needed in cosine mode for time resolved too.
            command = string.Format("TR={0},0,0", relativePosition_encCounts); // Triggers repeat ( for trapezoid mode)
          
            Console.WriteLine("  Arming for OTF trigger ... command to motor is: {0}", command);
            SendCommand(AxisAddress.ToString(), command, false, 0);     //  Set relative position to trip.
            SendCommand(AxisAddress.ToString(), "OS=3,28,0", false, 0);    //  Output of the motor in trip mode.
            SendCommand(AxisAddress.ToString(), "TE=0", false, 0);         //  Toggle Trip Enable
            SendCommand(AxisAddress.ToString(), "TE=16", false, 0);        //  Toggle Trip Enable  (Should be good to go !!)
        ...
        public int ToggleTriggerOut(int Axis, bool useGridRobot)
        {
            ...
            // PKv5.5.5.2
            SendCommand(AxisAddress.ToString(), "OS=3,16,0", false, 0);     //  Output of the motor in trip mode.
            SendCommand(AxisAddress.ToString(), "O3=1",false,0);            //  Toggle Trip Enable.
            Thread.Sleep(10);
            SendCommand(AxisAddress.ToString(), "O3=0", false, 0);         //  Toggle Trip Enable
            Thread.Sleep(10);
        ...
        public int SetAccel(int Axis, double accel_mm_per_s_s, double decel_mm_per_s_s, bool useGridRobot)
        {
            ...
            Console.WriteLine("Setting Accel and Decel Encoder Counts = {0}, {1}", accelEncCounts, decelEncCounts);
            Console.WriteLine("Axis 1 to 6 is: {0}", AxisAddress);
            SendCommand(AxisAddress.ToString(), command, false, 0);
            command = "D " + decelEncCounts.ToString();                                 //2015-04-28,  Bug Fix
            SendCommand(AxisAddress.ToString(), command, false, 0);
        ...
        public int SetVelocity(int Axis, double velocity_mm_s, bool useGridRobot)
        {
            ...
            SendCommand(AxisAddress.ToString(), command, false, 0);         // PKv5.5.5.2  Updated for new way,  forgot to comment out old way just deleted.
            return 0;
        }

        // 0 base Axis,  0 to 2
        // PKv4.0,2015-05-08
        // This routine will temporarily set VM to VI+10   (VM gets updated for every move,  so it's OK to set this value temporarily)
        ...
        public int SetVI(int Axis, int VI_counts, bool useGridRobot)
        {
            ...
            // PKv5.5.5.2
            SendCommand(AxisAddress.ToString(), "VI 1", false, 0); // Set a very low value temporarily avoid any issue with VM being less than VI
...

       {
            ...
            // PKv5.5.5.2 
           string resp = SendCommand((Axis + 1).ToString(), "PR ER", true, COMMAND_TIMEOUT);
           ...
           if (resp != "0")
           {
            ...
            // PKv5.5.5.2
            // Now clear the error.
            SendCommand((Axis + 1).ToString(), "ER 0", false, 0);
			...
			for (int Axis = 0; Axis < NUM_AXES; ++Axis)
			{
                if (ThisAxisSelected(AxisMask, Axis) & (Last_Commanded_Pos_mm[Axis] != Position_mm[Axis]))
                {
                    ...
                    try
                    {
                        ...
                        string command = "VM " + velocity.ToString();
                        SendCommand(AxisAddress.ToString(), command, false, 0);         // PKv5.5.5.2
 //                     if (SendCommand(AxisAddress.ToString(), command) != 0) return 1;   // Old Way
        ...
        // PKv5.5.5.2  TODO - Have not updated this routine yet to use the improved SendCommand() need to study a bit more
        //  on the differences when downloading a program.
...
//			if (unhandledMotionError) StopAllMotion();    //      PKv4.0,2015-05-08     Took out the stop.
 ...
 //           string resp = GetResponse(portIndex, COMMAND_TIMEOUT);
//           PKv5.5.5.2
             string resp = SendCommand((Axis + 1).ToString(), "PR C2", true, COMMAND_TIMEOUT);
             Convert.ToInt32(resp);
             return ConvertToMM(Convert.ToInt32(resp), AxisParams[Axis].DistResolution_mmPerEncCount);
        }

        //PKv5.5.5.2  Old way,  Used try / catch which is not neccessary with improved SendCommand()
        ...
        public int SetAxisPosition_mm(int Axis, double Position_mm)   // PKv4.0,2015-04-07
        {
            ...
            SendCommand((Axis + 1).ToString(), command, false, 0);
            int ret = 0;    // SendCommand does not return unless successful.
 ...
 //               resp = GetResponse(portIndex,COMMAND_TIMEOUT);
                ...
                // PKv5.5.5.2
                resp = SendCommand(AxisAddress.ToString(), "PR MV", true, COMMAND_TIMEOUT);
                ...
                // PKv5.5.5.2
                resp = SendCommand(AxisAddress.ToString(), "PR EF", true, COMMAND_TIMEOUT);
                ...
                if (resp == "1")
                {
                    ...
                    // resp = GetResponse(portIndex, COMMAND_TIMEOUT);
                    // PKv5.5.5.2
                    resp = SendCommand(AxisAddress.ToString(), "PR ER", true, COMMAND_TIMEOUT);
        ...
        // Can we talk to motion hardware and are all axis found ?
        // PKv3.0

        // PKv5.5.5.2  TODO - Have not updated this routine.  Not used much and would require some study.
