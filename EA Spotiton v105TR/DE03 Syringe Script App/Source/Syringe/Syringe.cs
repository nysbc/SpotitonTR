...
namespace Aurigin
{
	...
	public class SyringeParameters
	{
        ...
        public int DefaultSpeedCode                  // PKv4.2.4  Added 
        {
            get { return defaultSpeedCode; }
            set { defaultSpeedCode = value; }
        }
	...
	public class SyringeConfigurationData
	{
        ...
        public int      ComPortMultiplex;               //	indicate the total number of syringe pump  PKv4.1.1
	...
	public class UserSyringeControl
	{
        ...
        private static SyringeValvePosition[] lastValvePosition = new SyringeValvePosition[8];
...
//			Console.WriteLine("Syringe Pump Multiplexing set to " + pumpConfiguration.MultiplexPump.ToString());
		...
		public int InitializePump(int index)
		{
            ...
            DisplayVersion(pumpAddress); // PKv4.2.4
            ...
            lastValvePosition[index] = SyringeValvePosition.OutputPosition;      // PKv4.2.4

            return WaitForPumpMoveAndCheckStatus(pumpAddress, pumpConfiguration.TimeoutForSyringeMove_ms);
		}

        public int DisplayVersion(byte pumpAddress)
        {
            this.SendCommand(pumpAddress, "&");
            Thread.Sleep(100);
            string response;
            this.WaitForPumpResponse(100, 1, out response, pumpAddress.ToString());
            Console.WriteLine("    Syringe.cs: Pump Address={0}, Version ={1}", pumpAddress.ToString(), response);
            return 0;
        }
        ...
        public int SyringeDefaultSpeed(int index)
        {
            int SpeedCode = this.pumpConfiguration.SyringeParams[index].DefaultSpeedCode;
            return SpeedCode;
        }
        ...
        // Will turn valve and wait for completion
         
		public int SetValvePosition(int index, SyringeValvePosition position)
		{
            return SetValvePosition(index,position,true);
		}

        // If you wish to start to turn the valve without waiting for completion.
		...
		public int SetValvePosition(int index, SyringeValvePosition position, bool waitForCompletion)
		{
			...
			{
                lastValvePosition[index] = position;    // Assume it made it
                return 0;
			}

			// wait for the pump to be idle
            int ret = WaitForPumpMoveAndCheckStatus(pumpAddress, pumpConfiguration.TimeoutForValveMove_ms);
            lastValvePosition[index] = position;    // Assume it made it
            return ret;
		}
        ...
        public SyringeValvePosition GetValvePosition(int index)
        {
            try
            {
                SyringeValvePosition ret = lastValvePosition[index];
                return ret;
            }
            catch
            { return SyringeValvePosition.Unknown; }
        ...
        */
        ...
        private int InitComPort()
        {
            ...
            try
            {
                ...
                // PKv4.1.1,2015-07-16,  Cleaned this up to hopefully work better for multiplex and non-multiplex mode.

                if (!pumpConfiguration.MultiplexPump)   //PKv4.1,2015-07-17 
                {
                    for (int i = 0; i < pumpConfiguration.TotalSyringePump; i++)
                    {
                        ...
                        port[i] = pumpConfiguration.SyringeParams[i].PumpCommPort;   // Non-multiplex mode each pump has its own dedicated port.
                ...
                else   //  Multiplex mode,  Only uses one serial port. 
                {
                    port[0] = pumpConfiguration.ComPortMultiplex;
                    SerialPort serialPort = new SerialPort();
                    ManualResetEvent mutex = new ManualResetEvent(false);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("      Error initializing syringe com port at " + ex.ToString());
                return 1;
            }
