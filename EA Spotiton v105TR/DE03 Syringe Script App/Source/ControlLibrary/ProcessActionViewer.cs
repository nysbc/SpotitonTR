...
namespace ControlLibrary
{
	...
	//		System.Windows.Forms.DataGridCell
            ...
            // LEA FIX03
            // we want to display the grid such that it can't be modified by the user.
            CurrencyManager cm  = (CurrencyManager)this.BindingContext[dgProperties.DataSource, dgProperties.DataMember];
            ((DataView)cm.List).AllowNew=false;
            //((DataView)cm.List).AllowDelete=false;
            //((DataView)cm.List).AllowEdit=false;

            // LEA FIX03
            // We also want to prevent a field from remaining editable when the
            // cursor/focus is on the field.
            PreventEditing();
        ...
        private void PreventEditing()
        {
             ...
             foreach( Control c in dgProperties.Controls )
             {
                   if( c.GetType() == typeof( VScrollBar) || c.GetType() == typeof(HScrollBar) )
                          al.Add(c);
             }
             dgProperties.Controls.Clear();
             dgProperties.Controls.AddRange( (Control[])al.ToArray( typeof(Control) ) );
        }
